  ...
  return (
    <Animated.View style={[styles.position, { left: x - LEVEL_NODE_SIZE / 2, top: y - LEVEL_NODE_SIZE / 2 }, animatedStyle]}>
      ...
      >
        ...
        {statusIcon ? <View style={styles.statusBadge}><Ionicons color={level.status === "premium" ? "#7A5815" : colors.primary} name={statusIcon} size={11} /></View> : null}
      </Pressable>
    </Animated.View>
  );
}
...
const styles = StyleSheet.create({
  ...
  current: { borderColor: colors.primary, borderWidth: 3, shadowColor: colors.primary, shadowOffset: { height: 0, width: 0 }, shadowOpacity: 0.38, shadowRadius: 7 },
  currentLabel: { backgroundColor: colors.background, borderRadius: 9, color: colors.primary, fontFamily: fonts.bodySemiBold, fontSize: 9, left: 0, lineHeight: 13, overflow: "hidden", paddingVertical: 2, position: "absolute", textAlign: "center", top: -27, width: LEVEL_NODE_SIZE, zIndex: 2 },
  ...
  number: { color: colors.primary, fontFamily: fonts.bodySemiBold, fontSize: 16 },
  ...
  statusBadge: { alignItems: "center", backgroundColor: colors.background, borderColor: "rgba(54,75,38,0.2)", borderRadius: 8, borderWidth: 1, bottom: -7, height: 16, justifyContent: "center", left: (LEVEL_NODE_SIZE - 16) / 2, position: "absolute", width: 16 },
});
